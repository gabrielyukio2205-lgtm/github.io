...
export default defineConfig({
  plugins: [react()],
    ...
    // ========== VERCEL DEPLOY ==========
    function openInVercel() {
        // Works for both HTML and React modes
        if (currentMode === 'react' && Object.keys(currentFiles).length === 0) {
            alert('Gere um projeto React primeiro!');
            return;
        }
        ...
        if (currentMode === 'react') {
            ...
            // Ensure package.json with Vercel-compatible scripts
            if (!files['package.json']) {
                files['package.json'] = JSON.stringify({
                    name: 'jade-webdev-project',
                    version: '1.0.0',
...
export default defineConfig({
  plugins: [react()],
        ...
        // Create the project payload for Vercel's GitHub import
        // Since Vercel doesn't have a direct "paste files" URL like StackBlitz,
        ...
        const useCodeSandbox = confirm(
            '🚀 Deploy to Vercel\n\n' +
            'Para deploy no Vercel, vou abrir seu projeto no CodeSandbox primeiro.\n' +
            'De lá, você pode clicar em "Fork" → "Deploy" → "Vercel".\n\n' +
            'Continuar?'
        );
    ...
    function openInNetlify() {
        ...
        // For Netlify Drop, we need to create a zip file
        // Since we can't easily create zips in browser without a library,
        // we'll use Netlify's "Deploy to Netlify" button approach via GitHub
        // OR redirect to Netlify Drop with instructions

        if (currentMode === 'html') {
            // For HTML, Netlify Drop is perfect - just drag and drop
            // We'll open Netlify Drop and tell user to download + drag
            const proceed = confirm(
                '🌐 Deploy to Netlify\n\n' +
                'Para sites HTML estáticos:\n' +
                '1. Clique em "Download" para baixar seu site\n' +
                '2. Vou abrir o Netlify Drop\n' +
                '3. Arraste o arquivo HTML para a página\n\n' +
                'Continuar?'
            );
            ...
            if (proceed) {
                // Download file first
                downloadCode();
                // Then open Netlify Drop
                setTimeout(() => {
                    window.open('https://app.netlify.com/drop', '_blank');
                }, 500);
            }
        } else {
            // For React, use CodeSandbox as intermediary
            const files = {};
            Object.entries(currentFiles).forEach(([name, content]) => {
                const cleanName = name.startsWith('/') ? name.slice(1) : name;
                files[cleanName] = content;
...
export default defineConfig({
  plugins: [react()],
            ...
            const useCodeSandbox = confirm(
                ...
                'De lá, você pode exportar para GitHub e conectar ao Netlify.\n\n' +
                'Continuar?'
            );
    ...
    const netlifyBtn = document.getElementById('netlifyBtn');

    if (vercelBtn) {
        vercelBtn.addEventListener('click', openInVercel);
    }
    if (netlifyBtn) {
        netlifyBtn.addEventListener('click', openInNetlify);
    }
